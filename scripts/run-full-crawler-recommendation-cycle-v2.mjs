...
const resultDirectory = join(
  ...
  'full-crawler-recommendation-cycle-v2-result',
);
const overridePath = '/tmp/full-crawler-recommendation-cycle-v2.override.yml';

const directories = {
  dataset: '/app/apps/api/storage/build-decision-dataset-v3-full-crawler-20260724',
  contextualTraining:
    '/app/apps/api/storage/contextual-v3-training-full-crawler-v2-20260724',
  contextualCandidates:
    '/app/apps/api/storage/contextual-v3-candidate-evaluation-full-crawler-v2-20260724',
  bootstrap:
  ...
  value:
    '/app/apps/api/storage/recommendation-value-v4-full-crawler-v2-20260724',
  policy:
    '/app/apps/api/storage/recommendation-policy-v4-full-crawler-v2-20260724',
};
...
try {
  ...
  await prepareCleanDirectories();
  await configureApi();
  await waitForApi(endpoints.dataset + '/status', 20 * 60_000);
  await saveEnvironment();

  const datasetStatus = await requestJson('GET', endpoints.dataset + '/status');
  assertTrue(datasetStatus.state === 'COMPLETE', 'Full crawler dataset is not complete.');
  ...
  assertTrue(
    ...
    'The full crawler dataset excluded matches by limit.',
  );
  assertTrue(
    datasetManifest?.source?.selectedMatchCount === 22_848,
    `Unexpected full crawler match count: ${datasetManifest?.source?.selectedMatchCount}`,
  );
  const sourceSha256 = requiredString(datasetManifest, ['artifact', 'sha256']);
  ...
  assertTrue(
    trainingStatus.sourceMatchCount === datasetManifest.source.selectedMatchCount,
    `Contextual training used ${trainingStatus.sourceMatchCount} matches instead of ${datasetManifest.source.selectedMatchCount}.`,
  );
  assertTrue(
    trainingStatus.sourceRowCount === datasetManifest.artifact.rowCount,
    `Contextual training used ${trainingStatus.sourceRowCount} rows instead of ${datasetManifest.artifact.rowCount}.`,
  );
  assertTrue(
    trainingAudit?.source?.actualSha256 === sourceSha256,
    'Contextual training source SHA-256 does not match the full crawler dataset.',
  );
  ...
  assertTrue(
    candidateStatus.validationRowCount === trainingStatus.validationRowCount,
    'Candidate evaluation row count does not match contextual validation.',
  );
  assertTrue(
  ...
  assertTrue(
    bootstrapAudit?.source?.matchCount === trainingStatus.validationMatchCount,
    'Recommendation bootstrap match count does not match contextual validation.',
  );
...
async function prepareCleanDirectories() {
  const volumeRoot = resolveStorageVolumeRoot();
  const relativeDirectories = Object.entries(directories)
    .filter(([name]) => name !== 'dataset')
    .map(([, path]) => path.replace('/app/apps/api/storage/', ''));
  for (const relativeDirectory of relativeDirectories) {
    const hostPath = join(volumeRoot, relativeDirectory);
    commandOutput('sudo', ['rm', '-rf', hostPath]);
    commandOutput('sudo', ['mkdir', '-p', hostPath]);
  }
  await saveJson('storage-volume.json', {
    volumeRoot,
    preservedDatasetDirectory: directories.dataset,
    clearedDirectories: relativeDirectories,
  });
}
...
async function saveEnvironment() {
  ...
  const relevant = output
    .split('\n')
    .filter((line) =>
      line.startsWith('DEADLOCK_BUILD_DECISION_DATASET_V3_') ||
