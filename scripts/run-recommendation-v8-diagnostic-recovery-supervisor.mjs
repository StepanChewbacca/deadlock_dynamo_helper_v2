...
await main().catch(async (error) => {
  ...
  await writeStatus('FAILED', currentStage, {
    error: message,
    resolvedConfig,
  });
  process.exitCode = 1;
...
async function main() {
  ...
  await log(`Recommendation V8 diagnostic recovery supervisor started at ${new Date().toISOString()}.`);
  await log('Run mode is DIAGNOSTIC_NON_RELEASE. Current catalog fallback is explicitly authorized.');
  await waitForApi();
  ...
  const snapshotId = buildSnapshotId(
    chronology.trainingWindowEnd,
    catalog.id,
    source.manifest.artifact.sha256,
  );

  resolvedConfig = {
    runMode: 'DIAGNOSTIC_NON_RELEASE',
  ...
  await log(`Resolved diagnostic config: ${JSON.stringify(resolvedConfig)}`);
  ...
  await log(`Recommendation V8 diagnostic non-release run completed at ${new Date().toISOString()}.`);
}
...
async function loadCompletedSourceDataset() {
  ...
  await log(
    `Reusing Source Dataset V3: ${manifest.source.selectedMatchCount} matches and ${manifest.artifact.rowCount} decisions.`,
  );
...
async function deriveChronology(datasetPath) {
  ...
  const result = {
    ...
    trainingWindowEnd: ordered[trainingEndIndex].matchStartTime,
...
async function runPipeline({ source, chronology, catalog, snapshotId }) {
  const child = spawn('node', [PIPELINE_SCRIPT], {
    env: {
      ...process.env,
      API_BASE_URL,
      TRAINING_RUN_MODE: 'DIAGNOSTIC_NON_RELEASE',
...
function buildSnapshotId(trainingWindowEnd, catalogId, sourceSha256) {
  return `v8-diagnostic-${trainingWindowEnd.slice(0, 10).replaceAll('-', '')}-current-c${catalogId}-${sourceSha256.slice(0, 12)}`;
}
